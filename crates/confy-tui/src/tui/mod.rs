    ...
    while !should_quit {
        ...
        if let Event::Key(key) = event::read()? {
            ...
            if matches!(app.session.mode, crate::tui::state::Mode::Help(_)) {
                ...
                // width, content wraps within it (minus 2 borders + 4 padding each side/top-bottom).
                let size = terminal.size()?;
                let popup_width = (size.width * 65 / 100).min(size.width);
                let inner_w = popup_width.saturating_sub(6);
                let help_lines = ui::wrapped_line_count(&text, inner_w) as u16;
                let inner_h = size.height.saturating_sub(4);
