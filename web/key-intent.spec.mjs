...
{
  const r = resolve(normalMode, "E", { ctrl: false, shift: true });
  check('"E" -> BeginEditExternal, preventDefault (mirrors TUI E force-$EDITOR)', r?.kind === "intent" && r.intent === "BeginEditExternal" && r.preventDefault === true, JSON.stringify(r));
}
{
