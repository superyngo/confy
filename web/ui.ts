...
import type { Intent, ModeView, SessionSnapshot, ViewRow } from "./types.js";
...
function openText(text: string, format: "toml" | "json" | "yaml" | "yml") {
  session?.free();
  try {
...
function render() {
  ...
  if (snap.convert_write) doDownload(snap.convert_write[0], snap.convert_write[1]);
  if (snap.quit) {
...
function renderTree() {
  ...
  // Inline-edit mode: render the edit cell inside the cursor row.
  const edit =
    typeof snap!.mode === "object" && "Edit" in snap!.mode
      ? snap!.mode.Edit
      : null;
  ...
  for (const r of rows) {
    ...
    const marker = r.is_cursor ? "▶ " : "  ";
    const branch = r.is_branch ? (isExpanded(r) ? "▾ " : "▸ ") : "";
    let line = `<div class="row${r.is_cursor ? " cursor" : ""}${r.read_only ? " readonly" : ""}">`;
...
// We can't see the expand set from the snapshot directly; infer from the
// rendered rows: a branch is expanded iff any later row has it as a path prefix
// at depth+1.
...
/** The string discriminant of a `ModeView` (bare string for unit variants, the
 *  object key for tagged variants). */
...
function renderOverlay() {
  const m = snap!.mode;
  const tag = modeTag(m);
  // Normal / FilterResults / inline-Edit are handled in the tree itself.
  ...
  if (tag === "Detail") {
    ...
    overlay.innerHTML = `<h3>Type filter</h3><div>↑↓←→ move · Space toggle · Enter apply · Esc cancel</div>`;
  } else {
    overlay.classList.add("hidden");
  }
}
...
function onKey(ev: KeyboardEvent) {
  ...
  // If an external-edit modal is open, it owns input (handled in openExternalEdit).
  ...
  if (typeof m === "object" && "Prompt" in m) {
    if (ev.key === "y" || ev.key === "Y") return send({ PromptKey: "y" });
...
function doSave() {
  ...
  doDownload("confy-export" + extFor(snap!.doc_format), text);
  send("Save"); // mark saved in core
}

function extFor(f: string): string {
  return f === "Toml" ? ".toml" : f === "Json" ? ".json" : ".yaml";
}

function doDownload(filename: string, text: string) {
...
function bindGlobal() {
  ...
  document.body.addEventListener("keydown", (ev) => {
    // Redirect typing to the tree even when it lost focus.
    if (document.activeElement !== tree && noModalOpen()) {
      // Don't hijack buttons.
...
i              detail popup · ? this help · Ctrl-s save
