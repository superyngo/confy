...
Landed 2026-09-07 (CHANGELOG entry (4), commits `4b9ed34`..`98d6bb8`).
...
**Rationale:** `docs/reference/glossary.md:136-142` — in TOML, a comment sitting between a
...
/// re-parent that comment (docs/reference/glossary.md blank-line rule), so it confirms first.
#[test]
...
```rust
    ...
    /// (docs/reference/glossary.md blank-line rule), so it is confirmed first.
    BlankReparent {
        path: Path,
        n: usize,
    },
...
TOML's comment-ownership rule turns on the 0<->1 blank boundary (docs/reference/glossary.md),
...
- Modify: `docs/reference/MUTATIONS.md` (*Mutation mechanics* — a `SetTrailingBlankLines`
...
`docs/reference/MUTATIONS.md` *Mutation mechanics* must state: the anchor is the node's **contiguous extent
...
- **Why no ADR.** Unlike the datetime plan, nothing here reverses or narrows an existing
  ...
  lives in this plan plus docs/reference/MUTATIONS.md's *Mutation mechanics*.
