...
Model/TUI files modified (additive arms only): `model/node.rs` (Format variants), `model/document.rs` (KindTarget variants), `model/any_doc.rs`, `model/mod.rs` (already has `pub mod yaml`), `tui/app.rs` (`type_tag` + DocFormat thread), `tui/type_filter.rs` (`TypeToken`/`classify`/`Group`/`layout`/`nav_rows`), `tui/keys.rs` (`YAML_HELP`), `cli.rs` (drop the YAML bail), `CHANGELOG.md`/`CLAUDE.md`/`docs/reference/glossary.md`/`README.md`/`Cargo.toml`.
...
- [ ] **Step 3:** `docs/reference/glossary.md` — glossary: **Opaque node**, **Indent engine**, **YAML subset**, **core schema typing**.
