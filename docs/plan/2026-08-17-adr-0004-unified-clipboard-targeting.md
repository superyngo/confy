...
**Goal:** Make `PasteSlot` the one target representation for copy/cut/paste/move across TUI, web keyboard, web mouse, and touch (ADR 0004 §1-§2); fix the AoT-entry atomic-move data loss (§3) and sync `docs/reference/MUTATIONS.md` (§4).
...
- Node-kind/per-format legality is `docs/reference/MUTATIONS.md`'s job (ADR §4) — this plan updates the two spots ADR §3 changes (`Insert / move legality` notes, `Mutation mechanics` → Move row) and nothing else in that table.
...
## Phase 2: AoT atomic-move fix (§3) + `docs/reference/MUTATIONS.md` sync (§4)
...
### Task 6: `docs/reference/MUTATIONS.md` + `CHANGELOG.md` sync (§4)

**Files:**
- Modify: `docs/reference/MUTATIONS.md:294-299` (Insert / move legality note)
- Modify: `docs/reference/MUTATIONS.md:338` (Mutation mechanics → Move row)
- Modify: `docs/reference/glossary.md:202-209` (`PasteSlot` glossary entry's `After` wording)
- Modify: `CHANGELOG.md`
...
Re-read `docs/reference/MUTATIONS.md:290-300` first to get the current line numbers/tag (edits since Task 5 don't touch this file, but always re-anchor before a hash-line edit), then replace the note (currently starting `- ⏸ = an **AoT *group***...`):
...
Grilling this plan surfaced that `docs/reference/glossary.md`'s already-shipped `PasteSlot` glossary entry (from
...
fact `pointer_slot`'s top-band case (Task 2) depends on. Re-read `docs/reference/glossary.md:200-210` first to
...
Run: `grep -n "aot_entry_section_body" docs/reference/MUTATIONS.md && grep -n "first child instead" docs/reference/glossary.md`
...
```bash
git add docs/reference/MUTATIONS.md CHANGELOG.md
git commit -m "docs: sync docs/reference/MUTATIONS.md AoT-entry move description with the atomic-move fix (ADR 0004 §4)"
```
