...
- **Q5 (term):** the operation is canonically named **Reveal** — already recorded in `docs/reference/glossary.md` §Operations & projection (commit that edit in Task 5).
...
- `docs/reference/glossary.md` already carries the new **Reveal** glossary entry (added during the grilling session) — it is part of this work and is committed in Task 5.
...
// set cursor; docs/reference/glossary.md §Operations "Reveal") and closes the popup. The
...
- Commit: `docs/reference/glossary.md` (the **Reveal** glossary entry was already added during the grilling session — commit it here, no further edits)
...
- feat(web): breadcrumb bar + mini-tree picker below the filter row — segment
  ...
  excluded. New glossary term: Reveal (docs/reference/glossary.md §Operations).
```

- [ ] **Step 4: Commit docs + glossary (not CHANGELOG)**

```bash
git add WEBUI.md CLAUDE.md docs/reference/glossary.md
...
- Spec coverage: bar 渲染+glyph (T4 §4–7 / Q6)、segment 點擊=開迷你樹 (Q1+Q2/B, T4 §4)、迷你樹 lazy 展開+高亮+跳選 (T2 + T4)、filter 遮蔽提示 (Q4/C, T1 §5–6)、與主樹同節點集合 (Q3/A, T2 test 2)、glossary "Reveal" (Q5, docs/reference/glossary.md done + T5 commit)、VS Code host 可見性 (架構備註,零 extension 改動)、i18n (T1 §5, T4 §3)、docs/changelog (T5)。
