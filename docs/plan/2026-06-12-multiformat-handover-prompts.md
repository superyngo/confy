...
`docs/spec/2026-06-12-multiformat-backends-design.md`
...
執行 docs/plan/2026-06-12-phase1-backend-abstraction.md（Phase 1：後端抽象
化重構，spec 在 docs/spec/2026-06-12-multiformat-backends-design.md §Phase 1）。
...
（不用 agd 流程時，改成：`執行 docs/plan/2026-06-12-phase1-backend-abstraction.md，
...
```
為 confy 實作 JSON/JSONC 後端（spec：docs/spec/
...
- 測試：golden projection、tests/fixtures/*.json(c) byte-identical roundtrip、
  mutation 單元測試鏡像 cst_edit 套路;完成後更新 CHANGELOG/CLAUDE.md/docs/reference/glossary.md/README。
...
```
為 confy 實作 YAML 子集後端（spec：docs/spec/
...
```
為 confy 實作文件級格式轉換（spec：docs/spec/
