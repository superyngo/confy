...
**Spec:** `docs/spec/2026-08-20-vscode-outline-provider-design.md`
...
- `Node.text_range: Range<usize>` (not `span`) / `Node.key_text_range: Option<Range<usize>>`
  (not `key_span`) — deliberately distinct from `docs/reference/MUTATIONS.md`'s existing "Member
  spans" term.
...
`[T/D]` equivalent — `docs/reference/MUTATIONS.md`/the spec's Non-goals already exclude this
