...
- **Root `CHANGELOG.md` is growing at the pre-split rate again** (re-read 2026-09-18). The
  2026-09-09 split (F12) cut it to 119 KB by archiving v0.x verbatim under
  `docs/reference/changelog/`; nine days later it is **201 KB**, all of it `[Unreleased]` + v1.x.
  Nothing can be archived yet and nothing should be: `release.yml`'s `verify-versions` greps the
  **root** file for `## [vX.Y.Z]`, so archiving the series the next tag belongs to hard-fails a
  tagged build after the tag is pushed. **Trigger:** the first release of a **v2.x** series —
  then v1.x moves verbatim to `docs/reference/changelog/v1.x.md` with an index row, per
  `changelog/README.md`'s own rule.
...
| Closed | Item | Commit |
|---|---|---|
| 2026-09-18 | **The `?` help overlay printed a raw catalog key** — `help_legend_text`'s hardcoded row counts overshot the catalog by one in two formats (`[2, 6, 6]`/`[3, 7, 14]` against 5 `json.containers.*` and 6 `yaml.containers.*`), so `tr`'s documented "bug signal" branch leaked `tui.help.legend.json.containers.6` / `…yaml.containers.7` as a visible legend row on every JSON and YAML file in both languages. Counts corrected; the guard is exhaustive rather than a second copy of them — `legend_requests_no_missing_catalog_key` renders 3 formats × 2 languages and asserts no output contains `tui.help.legend.`. Reproduced and confirmed on the real binary both ways. Found by [`../audit/2026-09-18-documentation-audit.md`](../audit/2026-09-18-documentation-audit.md) §1 | closed by the commit that added this row (no hash: citing one from inside its own commit is the dangling-citation trap CLAUDE.md §Commit citations describes) |
| 2026-09-18 | **`help.row.convert` still said "(Root node)"** in both catalogs — and it is a *shared* key, so the phrasing ADR 0013 retired was on screen in the TUI `?` overlay and the web/touch help panel, two weeks after no host drew a Root row. The 2026-09-15 sweep struck the same wording from `README.md` and missed the catalog. Three more orphaned keys `adfc3b1` missed went with it (`tui.status.filter-results-status`, `web.common.confirm`, `web.help.title`); 433 → 430 keys each, parity intact | closed by the commit that added this row |
...
| 2026-09-15 | **The trailing-comment double pass** — closed as **stale**, not fixed. `edit_commit` splits the comment off before building the fragment, so `replace_value` returns `None` and `cst_edit::apply`'s `Mutation::Replace` arm takes its `None => tree` branch: a plain inline value edit on a commented Node never calls `set_trailing_comment`. No host commits per keystroke either (TUI on `Enter`, web on `Enter`/`blur`). The spec §1 **14× does not reproduce**: measured 2026-09-15 on 5000 sections (release, median of 9) — TOML `Replace` 227 ms with or without a trailing comment on the Node, `SetTrailingComment` 615 ms, `Replace` with a comment-bearing fragment 664 ms (2.7–2.9×, and only when the user changes a comment); YAML 59–61 ms on every route. Reopen only on a fresh measurement | `d9b7771` |
