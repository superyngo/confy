...
| 2026-09-14 | **In progress (plan `docs/plan/2026-09-11-root-hidden-alignment.md`, SD + S0 done).** **The TUI still draws the Root row — ADR 0013 is accepted but unimplemented.** `docs/spec/2026-09-11-root-hidden-alignment-design.md` is still `Draft` and has no task plan; only its sibling record (Raw write mode, its P3 entry point) shipped. So the promised alignment ("no host draws the Root") is not in the product: the TUI shows the `document file` row, and the two web hosts still carry the seven stand-ins that hide it. Reported by the user 2026-09-14 while reviewing the Raw-write ship. | `model/node.rs`'s `NodeTree::flatten` still walks from `self.root`; `tui/ui.rs`'s `draw_title` reads `app.rows.first()`; `web/render.ts` + `web/touch/render.ts` still filter row 0 and rebase depth; `web/path-utils.ts`'s `drawnCursorFallback`/`overshotUndrawnRootSlot` still exist. | M | The design record is `Approved` with a task plan under `docs/plan/`, and slices S0–S6 land: no host emits or draws a Root row, the deleted host stand-ins return no `rg` hits, and a real-binary TUI pass shows the first row as a top-level Node with the title bar still naming the file. |
