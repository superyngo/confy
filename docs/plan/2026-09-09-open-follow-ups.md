...
| 2026-09-15 | **A YAML multi-line comment block's extent stops after its first line.** Consecutive `#` lines project as ONE Comment node, but the index stores only the block's **first** token and the projected node's `text_range` covers only that first line. `yaml/edit/resolve.rs`'s `extent_end_offset` then returns the first line's end — while its own doc-comment claims "the token spans the block already (the lexer merges the run)". So `trailing_blank_anchor` (and the Detail popup's `Blank after:` readout) for a 2+-line YAML comment block measures the blanks after line 1, not after the block. | `yaml/project.rs`'s `flush_comment_block` pushes `Target::Comment(first_tok)` and sets `text_range` from that token; `yaml/edit/resolve.rs`'s `Target::Comment` arm. Found while implementing `node_text_spans`, which works around it with its own `comment_block_end` walk (`yaml/edit/spans.rs`). TOML has no such gap — it uses `comment_block_range`. | S | `extent_end_offset` walks the run (or the index/projection records the block's real range), the workaround in `yaml/edit/spans.rs` is deleted, and a test pins `trailing_blank_lines` on a 2-line YAML comment block followed by blanks. |
| 2026-09-15 | **A rejected Block reports a document-space offset, not a buffer one.** `apply_block_text` splices the buffer into the whole file and reparses, so a parse error's offset is in *document* space. Hosts show the message but cannot place the caret on the offending line of the buffer the user is still holding. | `session/inline_edit.rs`'s `apply_block_text` (the `Err` arm passes `e.to_string()` straight through); design record §11 lists it as deliberately out of scope. | S | The commit path translates an offset inside the spliced region back to a buffer offset, and at least one host (the web pop-up) moves the caret there on rejection. |
| 2026-09-15 | **`Block` and `Remark` (`r`) now overlap, and the glossary does not say how.** Remarking a Node whose Block spans several lines (a `[table]`, a multi-line array) comments the whole extent, which is the Block — but the two terms were defined independently and neither entry references the other. | `docs/reference/glossary.md` (**Block**, **Remark**); `MUTATIONS.md`'s Remark row. No behavior is wrong today; the vocabulary is under-specified. | XS | Both glossary entries state the relationship (Remark operates on the Block's lines), and `MUTATIONS.md`'s Remark row uses the term. |
| 2026-09-15 | **The trailing-comment double-pass still costs 14× for an *inline* editor commit.** `cst_edit/mod.rs:81` re-serializes to extract a node's EOL comment. The Block route bypasses it entirely (it commits one whole-document `Replace`), so the remaining exposure is the inline editor's per-keystroke-commit path on a node that has a trailing comment. | `crates/confy-core/src/model/cst_edit/mod.rs:81`; measured as defect 3 in `docs/spec/2026-09-15-block-edit-whole-file-reparse-design.md` §1. | M | The trailing comment is read off the green tree without a second serialize, and the §1 measurement is re-taken and recorded. |
