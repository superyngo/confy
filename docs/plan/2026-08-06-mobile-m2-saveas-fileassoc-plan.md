...
Decisions grilled and locked 2026-08-06, see
[`../adr/0001-android-save-as-persistable-grant.md`](../adr/0001-android-save-as-persistable-grant.md).

**Spec:** none separate — this plan's own "Governing facts" section is the spec, distilled from
`docs/plan/2026-07-13-mobile-m1-android-plan.md` (M1, shipped 2026-07-15) and a
...
- **M1 plan** `docs/plan/2026-07-13-mobile-m1-android-plan.md` — full history.
...
- iOS (not targeted per `docs/reference/RELEASES.md`).
...
2. Insert new, hand-authored `<intent-filter>` blocks immediately after that closing marker (still
   ...
   <!-- MANUAL, confy-specific: survives `cargo tauri android build` regeneration because it's
        ...
        visibility, and docs/plan/2026-08-06-mobile-m2-saveas-fileassoc-plan.md.
...
6. `TAURI.md` § Mobile (Tauri Android): remove/update the `canSaveAs()` gating paragraph (no
   ...
   rationale.
7. `docs/reference/RELEASES.md`: Android Google Play row — drop "另存新檔/Save-As still unimplemented" from the
...
```
請用 executing-plans skill 執行 docs/plan/2026-08-06-mobile-m2-saveas-fileassoc-plan.md
