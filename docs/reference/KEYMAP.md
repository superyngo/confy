...
| `⌘Z` / `Ctrl+Z`, `⇧⌘Z` / `⌘Y` / `Ctrl+Y` | Undo / redo **inside the buffer** | The `<textarea>`'s own history (`document.execCommand`), not core's — core refuses `Undo`/`Redo` while the document buffer is open (R24). Handled by confy rather than left to the browser **because VS Code's webview preload swallows these keys** and routes them to the workbench's `undo` command, which never reaches the textarea (measured 2026-09-17: ⌘X/C/V work in that host, ⌘Z/Y did nothing). A programmatic `.value` write clears the native stack, so buffer undo can never cross an Apply — which is what keeps R24's guarantee intact. The toolbar's ↶/↷, the `⋯` rows and Tauri's Edit menu take the same route while write mode is open (`uiUndo`/`uiRedo`); bare `z`/`y` keep typing characters |
