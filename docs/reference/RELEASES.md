...
| Desktop app (macOS `.dmg`, Windows portable `.exe`) | GitHub Releases | same workflow, tag `v[0-9]*.[0-9]*.[0-9]*` | v1.3.0 | Live — unsigned/un-notarized (see README § Desktop app) |
...
| VS Code extension | VS Marketplace + Open VSX | `.github/workflows/publish-vscode.yml`, dispatched by `publish-gate.yml` after `release.yml` succeeds on tag `v[0-9]*.[0-9]*.[0-9]*`, gated behind its own `publish-gate-vscode` environment approval (versioned in lockstep with the app) | v1.3.0 | Live |
