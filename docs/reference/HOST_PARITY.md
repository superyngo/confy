...
| Whole-document (empty-path) edit | reachable by moving the cursor onto the Root row, same handshake as any node | desktop: Raw pane **write mode** (`rawState: "write"`, a `<textarea>`), entered from the Action menu's *Edit whole file as text*; touch: the same external-edit **bottom sheet** a per-node multiline edit uses, with the empty path as one more case (no write mode of its own) | web hides the Root row (§2), so it needs its own entry point; touch has no room for a control band | `docs/spec/2026-09-11-raw-write-mode-design.md` R1–R4, R18–R19; [WEBUI.md](WEBUI.md) §Tree \| Raw view \| Raw write |
...
| Whole-document edit's Edit control | n/a | suppressed under VS Code (Raw pane's Edit button, Action menu's *Edit whole file as text*) — the webview's own `TextDocument` is already this feature's one owner | a second editable copy would be two owners of one document | [ADR 0007](../adr/0007-vscode-schema-session-in-place-replace.md); `docs/spec/2026-09-11-raw-write-mode-design.md` R10 |
