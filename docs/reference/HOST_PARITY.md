...
| Whole-document edit's Edit control | n/a | suppressed under VS Code — the Action menu item is absent and the band's View ⇄ Edit toggle is **disabled** (not hidden, so the band keeps its static geometry) — the webview's own `TextDocument` is already this feature's one owner | a second editable copy would be two owners of one document | [ADR 0007](../adr/0007-vscode-schema-session-in-place-replace.md); `docs/spec/2026-09-11-raw-write-mode-design.md` R10 |
