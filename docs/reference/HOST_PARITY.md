...
| Chrome trimming | status bar + column header | VS Code and Tauri hide header row 1 / Undo-Redo and relocate the Raw-Tree toggle, deferring to the native menu & workbench | never duplicate a native control | [CHROME.md](CHROME.md) §Per-host chrome trimming; [TAURI.md](TAURI.md); [VSCODE.md](VSCODE.md) |
...
| Breadcrumb bar | none (title/status carry the path) | desktop, Tauri, VS Code only — deliberately **absent on touch**; stays visible and live in both Raw view **and** Raw write, and the mapping is **two-way** since 2026-09-15: a pick selects the node's source span in the Raw pane, and a Raw caret move reveals the Node the caret sits in (`node_at_offset`, latch + debounce + identity guards) | touch is sheet-driven with a weak cursor concept | [WEBUI.md](WEBUI.md) §Breadcrumb bar, §Tree \| Raw view \| Raw write; `docs/spec/2026-09-11-raw-write-mode-design.md` R12, R14–R17 |
...
| Whole-document edit's Edit control | n/a — the TUI uses `$EDITOR` instead (R9) | the Action menu item and the crumbs-row band's primary control open the Raw pane's write mode, on desktop web **and** VS Code alike since ADR 0015 (touch keeps its own sheet, R18) | the TUI has no multi-line text widget of its own | [ADR 0014](../adr/0014-whole-document-editing-reuses-each-hosts-text-surface.md); [ADR 0015](../adr/0015-vscode-raw-pane-write-mode-enabled.md); `docs/spec/2026-09-11-raw-write-mode-design.md` R9, R18 |
| Save As / Convert to a **new** file | always available | available on every platform since M2 (Android goes through `tauri-plugin-confy-picker`'s `create_writable`, which takes a persistable write grant); the web falls back to a download when the File System Access API is absent | stock `tauri-plugin-dialog`'s Android save dialog never took a write grant, so M1 had `canSaveAs()` hardcoded `false` | [TAURI.md](TAURI.md) §Mobile (Tauri Android); [ADR 0001](../adr/0001-android-save-as-persistable-grant.md) |
