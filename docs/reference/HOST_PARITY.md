...
| **Transitional** — multi-line editor commit route | `Intent::ApplyBlockText`: the buffer is the Node's **Block**, so it may rename the key or emit several siblings | `Intent::ApplyReplace`: still the old 1:1 per-Node fragment | the core route shipped with the TUI first so it could be verified on a real binary. **Remove this row** when the web hosts migrate (plan `2026-09-15-block-edit-whole-file-reparse-plan.md` Task 7) | [BEHAVIOR_MATRIX.md](BEHAVIOR_MATRIX.md) §6.3 |
