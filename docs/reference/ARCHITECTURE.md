# Architecture — workspace shape, module map, and dependency surface

Where the code lives: which crate owns what, which side of the core/host seam a responsibility
sits on, every module in the tree, and what the third-party surface is. Behavior contracts live
in the sibling reference documents ([`README.md`](README.md) indexes them); this file answers
*where does this code live* and *what does it depend on*. The conduct file
[`../../CLAUDE.md`](../../CLAUDE.md) holds the commands, the release process, and the two
review-gate invariants this layout exists to protect.
...
[`../spec/2026-06-17-headless-core-port.md`](../spec/2026-06-17-headless-core-port.md)):
...
crates/confy-ffi/         Stage-2 WASM wrapper over confy-core (wasm-bindgen + serde-wasm-bindgen)
  ...
  functional_smoke.mjs     node verification of the Intent→snapshot contract (176 checks)
...
web/                       TypeScript integration + **web-native** UI (see WEBUI.md) — a
  ...
  ui.ts          orchestrator: holds the latest snapshot, renders via render.ts + the modal
                 ...
                 crumbs-row band of two same-size controls (the primary action, labelled
                 Edit in Raw view and Apply in Raw write, plus Cancel),
...
crates/confy-tauri/       desktop + Android app shell (Tauri v2) over the web UI — **native file
  ...
  build.rs         `tauri_build::build()`; msix/ holds the Microsoft Store packaging inputs
                   (incl. the `listings/listingData-*.csv` release notes the release process
                   updates) and play/ the Google Play listing material.
...
crates/tauri-plugin-confy-picker/   first-party Tauri mobile plugin, Android-only real
  ...
  src/{lib,commands,desktop,mobile,error}.rs  plugin init + the two commands
                 (`pick_writable`, `create_writable`) with a mobile impl and a desktop stub;
                 guest-js/ + permissions/ + build.rs are the standard Tauri plugin scaffolding.
...
the Android-specific forks: `pickOpenFile()` calls `plugin:confy-picker|pick_writable` instead of
`dialog.open()`, and `pickSaveFile()` calls `plugin:confy-picker|create_writable` instead of
`dialog.save()` (see the crate note above) — so `canSaveAs()` is `true` on every platform since
M2; [`TAURI.md`](TAURI.md) §Save As records why M1 had it hardcoded `false`.
`fileAssociations` + `opened_urls`/`"opened"` deliver a file picked from Android's "Open with"
chooser through the same `openTauriPath`-style read path.

`confy-core` is pure and **filesystem-free at runtime** (see [`../../CLAUDE.md`](../../CLAUDE.md) §Architecture — the review-gate invariants); the host owns
...
## Dependency surface — `taplo` is unmaintained upstream

The maintainer stepped down in Dec 2024
...
vendoring scope. Estimate last measured **2026-09-15** (1,241 LOC exact, taplo 0.14.0);
recount before acting on it.
