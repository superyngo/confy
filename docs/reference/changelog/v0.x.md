# Changelog archive — v0.x

Pre-1.0 releases of confy, moved out of the root [`CHANGELOG.md`](../../../CHANGELOG.md) so it
carries only `[Unreleased]` and the current (v1.x) series. Format and ordering are unchanged
(newest first); nothing was edited on the way over.

Index: [changelog archives](README.md).
