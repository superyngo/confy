...
| **Replace** | Empty path = **whole-document reparse** (rejects invalid as `Fragment`; doc untouched) — the *whole-document editing* route (`Session::apply_document_text`): TUI `$EDITOR`, desktop Raw write mode, touch's external-edit sheet (ADR 0014). An `[A/T]`-entry path (`product[0]`) rewrites only that `[[…]]` entry; sibling entries and between-entry comments stay intact. The fragment must resolve to **exactly one** value node in every backend. JSON and YAML reject surplus text as `Fragment` (F14 — it mattered for YAML, whose lenient grammar happily parses a two-node fragment). TOML rejects a fragment whose **key** differs from the path's key (the synthetic carriers `__k__`/`__elem__` mean "no key" and stay exempt), but still drops surplus *nodes* after the first silently. That surplus gap no longer reaches a user: the multi-line editor commits a **Block** through the empty path instead ([BEHAVIOR_MATRIX.md](BEHAVIOR_MATRIX.md) §6.3), where N nodes are legal rather than surplus, so a non-empty-path `Replace` is now only the inline editor's one-value fragment. |
...
| **Remark / EditComment / InsertComment** | Comments are first-class — see *Comment* / *Trailing comment*. Remark comments out the lines of the Node's **Block** (that Block's trailing blank run excluded), so it applies to any node that **occupies its own line(s)** — a keyed member, an **array element**, a whole table/section — in all three formats, and un-remarking restores the source byte-for-byte. It does not apply inside a **single-line** collection (`a = [1, 2]`, `{"x": 1, "y": 2}`, `a: {x: 1}`), where the Block is a token range sharing its line and a comment leader would swallow the siblings, nor to a read-only node; every backend reports that case as `Unsupported`. Pinned by `tests/format_parity.rs`; the rule itself is [BEHAVIOR_MATRIX.md](BEHAVIOR_MATRIX.md) §8. Only a **non-blank** line of an `EditComment`/`InsertComment` text must carry the `#`/`//` marker: a blank line is the authored separator that splits the block into one projected Comment node **per group**, so the multiline editor applies its packaged trailing run to the **last** group rather than to `path`, which after such a splice names only the first (`blank_lines::blank_separated_groups`). |
