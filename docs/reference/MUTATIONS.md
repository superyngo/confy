...
| **Replace** | Empty path = **whole-document reparse** (rejects invalid as `Fragment`; doc untouched) — the *whole-document editing* route (`Session::apply_document_text`): TUI `$EDITOR`, desktop Raw write mode, touch's external-edit sheet (ADR 0014). An `[A/T]`-entry path (`product[0]`) rewrites only that `[[…]]` entry; sibling entries and between-entry comments stay intact. The fragment must resolve to **exactly one** value node in every backend; surplus text is rejected as `Fragment`, never silently dropped (F14 — it mattered for YAML, whose lenient grammar happily parses a two-node fragment). |
