...
pub struct Node {
    ...
    /// (not `span`) deliberately — `docs/reference/MUTATIONS.md` already defines **Member
...
- **`Format::Dotted` synthetic Table nodes** (`[T/D]`, e.g. the merged `a`
  ...
  "first definition position" convention `docs/reference/MUTATIONS.md`'s Dotted-table entry
...
- **Any Table whose descendant sub-sections are defined non-adjacently**
  (e.g. `[fruit]` … `[other]` … `[fruit.apple]`, per `docs/reference/MUTATIONS.md`'s general
