...
canonical docs/reference/glossary.md terms (§ Schema), explicitly contrasted with confy's existing
...
- `schema/value_bridge.rs` — converts confy's format-neutral `Value` tree
  ...
  is already taken by `model/value.rs`'s own tree; see `docs/reference/glossary.md` § Schema), plus a
  ...
  - YAML has no datetime type by confy's own design (docs/reference/glossary.md: "a date- or time-looking
...
async-signal pattern already established for `external_edit` (§8.2 in docs/spec/2026-06-17-headless-core-port.md) and
...
- **Document-level summary**: `SchemaStatus.violation_count` surfaces through each
  ...
  existing `Reveal` concept (docs/reference/glossary.md) rather than a new navigation primitive.
