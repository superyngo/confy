...
| VS Code extension | VS Marketplace + Open VSX | `.github/workflows/publish-vscode.yml`, dispatched by `publish-gate.yml` after `release.yml` succeeds on tag `v*.*.*`, gated behind its own `publish-gate-vscode` environment approval (versioned in lockstep with the app) | v0.31.1 | Live |
